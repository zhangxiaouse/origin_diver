...
void localizationCallback(const nav_msgs::OdometryConstPtr &msg){
    ...
    if (ros::Time::now().toSec() - now_time >= 0.05)
    {
        ...
        ROS_INFO("Write localization data num %u to %s.", localization_count, serial_port.c_str());
    }
...
//     speed_data_msg.speed_cmps = (dataSub.speed_left + dataSub.speed_right) * 0.1 * 0.5;
//     speed_data_msg.speed_kmph = (dataSub.speed_left + dataSub.speed_right) * 0.001 * 0.5 * 3.6;
...
void imu_data_tomsg(){
    ...
    imu_data_msg.angular_velocity.z = (dataSub.speed_right - dataSub.speed_left) * 0.001 / wheel_distance; // 0.64 = distance of two wheels
...
void speed_data_handle(ros::Publisher pubSpeed, ros::Publisher pubImu)
{
    ...
    if (recv_speed_data[0]==0x41 && ((recv_speed_data[18]<<8)|recv_speed_data[19])==crc16(recv_speed_data, 18))
    {

        uint16_t temp_16_s = (recv_speed_data[1]<<8) | (recv_speed_data[2]);
        dataSub.localization_num = temp_16_s;
        uint32_t temp_32_s = (recv_speed_data[3]<<24) | (recv_speed_data[4]<<16) | (recv_speed_data[5]<<8) | (recv_speed_data[6]);
        dataSub.speed_left = int(temp_32_s);
        temp_32_s = (recv_speed_data[7]<<24) | (recv_speed_data[8]<<16) | (recv_speed_data[9]<<8) | (recv_speed_data[10]);
        dataSub.speed_right = int(temp_32_s);
        temp_32_s = (recv_speed_data[11]<<24) | (recv_speed_data[12]<<16) | (recv_speed_data[13]<<8) | (recv_speed_data[14]);
...
int main(int argc, char *argv[])
{
    ...
    std::cout << serial_port <<std::endl;std::cout << serial_port <<std::endl;
    ...
    // ros::Publisher pubSpeed = n.advertise<cyber_msgs::SpeedFeedback>(speed_topic, 1000);
    ros::Publisher pubSpeed = n.advertise<cyber_msgs::SpeedFeedbackAGV>(speed_topic, 1000);
    ros::Publisher pubImu = n.advertise<sensor_msgs::Imu>(imu_topic, 1000);
    ros::Subscriber sublocalization = n.subscribe(localization_topic, 1, localizationCallback);
