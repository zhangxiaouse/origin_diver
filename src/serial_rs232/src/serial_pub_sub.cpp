...
std::string perception_emergency_topic = "/perception/emergency";
std::string localization_topic         = "/localization/estimation_odom";
double      wheel_distance             = 0.64;

class SerialPort {
 public:
  SerialPort(const std::string& port, int baudrate) : port_(port), baudrate_(baudrate) {
    ser_.setPort(port_);
...
std::shared_ptr<SerialPort> serial_ptr;
...
void send_to_senddata(const Datapub& dataPub, std::vector<unsigned char>& buff) {
  ...
  std::vector<unsigned char> send_localization_vector(send_localization_data, send_localization_data + 20);
  buff = std::move(send_localization_vector);
}

void perceptionEmergencyCallback(const std_msgs::Int8ConstPtr& msg) {
  if (msg->data != 0) {
    dataPub.stop_flag = 1;
  } else {
    dataPub.stop_flag = 0;
  ...
  while (ros::ok()) {
    ...
    while (read_buffer.size() >= TARGET_LENGTH) {
      ...
      if (read_buffer.size() >= TARGET_LENGTH) {
        ...
        std::move(read_buffer.begin(), read_buffer.begin() + TARGET_LENGTH, parse_speed_data.begin());
        read_buffer.erase(read_buffer.begin(), read_buffer.begin() + TARGET_LENGTH);
        if (((parse_speed_data[18] << 8) | parse_speed_data[19]) == crc16(parse_speed_data.data(), 18)) {
          std::lock_guard<std::mutex> lock(dataSub_mtx);

          dataSub.localization_num = (uint16_t)((parse_speed_data[1] << 8) | parse_speed_data[2]);
          dataSub.speed_left =
            (int32_t)((parse_speed_data[3] << 24) | (parse_speed_data[4] << 16) | (parse_speed_data[5] << 8) | parse_speed_data[6]);
          dataSub.speed_right =
            (int32_t)((parse_speed_data[7] << 24) | (parse_speed_data[8] << 16) | (parse_speed_data[9] << 8) | parse_speed_data[10]);
          dataSub.timestamp =
            (int32_t)((parse_speed_data[11] << 24) | (parse_speed_data[12] << 16) | (parse_speed_data[13] << 8) | parse_speed_data[14]);
          read_buffer.clear();
        } else {
...
void send() {
  while (ros::ok()) {
    ...
    if (dataPub.isNew == true) {
      ...
      for (int i = 0; i < send_localization_vector.size(); i++) {
        std::cout << std::to_string(send_localization_vector[i]) << "  ";
      }

      // std::cout << " " << std::endl;
      serial_ptr->write(send_localization_vector);
      dataPub.isNew = false;
    }
    if (ros::Time::now().toSec() - last_time >= 0.05) {
      ROS_WARN("Localization data lost about : %lfs.", ros::Time::now().toSec() - last_time);
    }

    {
      ...
      speed_data_msg.angular_velocity = (dataSub.speed_right - dataSub.speed_left) * 0.001 / wheel_distance;
      pubSpeed.publish(speed_data_msg);
      std::cout << " publish ros data [" << dataSub.speed_left << ", " << dataSub.speed_right << "] " << std::endl;
    }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << serial_port << "1" << std::endl;

  pubSpeed = n.advertise<cyber_msgs::SpeedFeedbackDiff>(speed_topic, 100);

  auto sublocalization     = n.subscribe(localization_topic, 1, localizationCallback);
  auto perceptionEmergency = n.subscribe(perception_emergency_topic, 1, perceptionEmergencyCallback);

  serial_ptr = std::make_shared<SerialPort>(serial_port, baudrate);
  if (!serial_ptr->open()) {
    return -1;
  }
